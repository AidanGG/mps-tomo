...
def computeFidelity(labExpVals,mpsExpVals,maxPauliChains,m,paulis,numQubits):
    ...
    print("Traces: MPS: " + str(np.trace(rhoMpsApprox)) + " lab: " + str(np.trace(rhoActApprox)) + "\n")
    print(rhoActApprox)
    print(rhoMpsApprox)
...
def estimateFidelity(numQubits,rhoMps,numSamples,opSpaceDim):
    ...
    #print(mpsExpsVals)
    fidEst = computeFidelity(mpsExpVals,mpsExpVals, pauliChains, numSamples,paulis,numQubits)
    return fidEst
